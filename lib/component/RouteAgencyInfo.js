import PropTypes from 'prop-types';
import React from 'react';
import Relay from 'react-relay';
import get from 'lodash/get';
import AgencyInfo from './AgencyInfo';

function RouteAgencyInfo(_ref, _ref2) {
  var route = _ref.route;
  var config = _ref2.config;

  var agencyName = get(route, 'agency.name');
  var url = get(route, 'agency.fareUrl') || get(route, 'agency.url');
  var show = get(config, 'agency.show', false);

  if (show) {
    return React.createElement(
      'div',
      { className: 'route-agency' },
      React.createElement(AgencyInfo, { url: url, agencyName: agencyName })
    );
  }return null;
}

RouteAgencyInfo.contextTypes = {
  config: PropTypes.object.isRequired
};

RouteAgencyInfo.propTypes = {
  route: PropTypes.object
};

export default Relay.createContainer(RouteAgencyInfo, {
  fragments: {
    route: function route() {
      return function () {
        return {
          children: [{
            children: [{
              fieldName: 'name',
              kind: 'Field',
              metadata: {},
              type: 'String'
            }, {
              fieldName: 'url',
              kind: 'Field',
              metadata: {},
              type: 'String'
            }, {
              fieldName: 'fareUrl',
              kind: 'Field',
              metadata: {},
              type: 'String'
            }, {
              fieldName: 'id',
              kind: 'Field',
              metadata: {
                isGenerated: true,
                isRequisite: true
              },
              type: 'ID'
            }],
            fieldName: 'agency',
            kind: 'Field',
            metadata: {
              canHaveSubselections: true,
              inferredRootCallName: 'node',
              inferredPrimaryKey: 'id'
            },
            type: 'Agency'
          }, {
            fieldName: 'id',
            kind: 'Field',
            metadata: {
              isGenerated: true,
              isRequisite: true
            },
            type: 'ID'
          }],
          id: Relay.QL.__id(),
          kind: 'Fragment',
          metadata: {},
          name: 'RouteAgencyInfo_RouteRelayQL',
          type: 'Route'
        };
      }();
    }
  }
});
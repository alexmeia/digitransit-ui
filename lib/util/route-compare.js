export default function routeCompare(routea, routeb) {
  if (routea.agency && routea.agency.name && routeb.agency && routeb.agency.name) {
    var agencyCompare = routea.agency.name.localeCompare(routeb.agency.name);
    if (agencyCompare !== 0) {
  ...
  return numberA - numberB || (routea.shortName || '').localeCompare(routeb.shortName || '') || (routea.longName || '').localeCompare(routeb.longName || '');
}
...
var geoWatchId = void 0;

function reverseGeocodeAddress(actionContext, location) {
  var language = actionContext.getStore('PreferencesStore').getLanguage();
  ...
  return getJson(actionContext.config.URL.PELIAS_REVERSE_GEOCODER, {
    ...
    if (data.features != null && data.features.length > 0) {
      var match = data.features[0].properties;
...
var runReverseGeocodingAction = function runReverseGeocodingAction(actionContext, lat, lon, done) {
  return actionContext.executeAction(reverseGeocodeAddress, {
    lat: lat,
    lon: lon
  }, done);
};

var debouncedRunReverseGeocodingAction = debounce(runReverseGeocodingAction, 60000, {
  leading: true
});

var setCurrentLocation = function setCurrentLocation(actionContext, position) {
...
export function geolocatonCallback(actionContext, _ref, done) {
  var pos = _ref.pos,
      disableDebounce = _ref.disableDebounce;
  ...
  if (disableDebounce) {
    ...
    debouncedRunReverseGeocodingAction(actionContext, pos.coords.latitude, pos.coords.longitude, done);
  }
}

function dispatchGeolocationError(actionContext, error) {
  if (!(actionContext.getStore('EndpointStore').getOrigin().userSetPosition || actionContext.getStore('PositionStore').getLocationState().hasLocation)) {
    switch (error.code) {
      case 1:
        actionContext.dispatch('GeolocationDenied');break;
      case 2:
        actionContext.dispatch('GeolocationNotSupported');break;
      case 3:
        actionContext.dispatch('GeolocationTimeout');break;
      default:
        break;
    }
  }
...
function watchPosition(actionContext, done) {
  ...
  var timeout = setTimeout(function () {
    return actionContext.dispatch('GeolocationWatchTimeout');

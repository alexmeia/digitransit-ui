  ...
  if (isBrowser && window.localStorage) {
    try {
      ...
      if (error.name === 'QuotaExceededError') {
        ...
        handleSecurityError(error, '[localStorage]' + ' Unable to save state; access to localStorage denied by browser settings');
      }
    }
  }
}
...
export function setCustomizedSettings(data) {
  ...
  var oldSettings = getCustomizedSettings();
  var newSettings = {
    accessibilityOption: !(typeof data.accessibilityOption === 'undefined') ? data.accessibilityOption : oldSettings.accessibilityOption,
    minTransferTime: data.minTransferTime ? data.minTransferTime : oldSettings.minTransferTime,
    modes: data.modes ? data.modes : oldSettings.modes,
    walkBoardCost: data.walkBoardCost ? data.walkBoardCost : oldSettings.walkBoardCost,
    walkReluctance: data.walkReluctance ? data.walkReluctance : oldSettings.walkReluctance,
...
export function setPositioningHasSucceeded(state) {
  setItem('positioningSuccesful', { state: state });
}
